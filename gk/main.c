	...
	struct gatekeeper_if *other_side)
{
	...
	/* Use the GK block to proxy connections through Amazon. */
	uint8_t last_octet = 0;
	(void)last_octet;
	if (tcp_hdr != NULL) {
		last_octet = ntohs(tcp_hdr->src_port) & 0x00FF;
	} else if (udp_hdr != NULL) {
		last_octet = ntohs(udp_hdr->src_port) & 0x00FF;
	}
	...
	struct gk_instance *instance, struct gk_config *gk_conf)
{
	...
	for (i = 0; i < num_rx; i++) {
		...
		ret = extract_packet_info(pkt, packet, NULL);
		if (ret < 0) {
			if (likely(packet->flow.proto == RTE_ETHER_TYPE_ARP)) {
				arp_bufs[num_arp++] = pkt;
				continue;
